...
In @tip-dplyr-vs-base-r there are two examples of the same analysis, one in base R and one using dplyr. You'll notice the numbers are the same, but that one returns a regular dataframe and the other returns a 'tibble' (if you want you can verify this by copying and pasting the corresponding code). Tibbles are also part of the tidyverse, take a look at the [tibble homepage](https://tibble.tidyverse.org/) and describe the main differences with regular dataframes.
:::
