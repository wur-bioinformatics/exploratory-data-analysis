...
__Debugging__ is the process of identifying, understanding, and fixing these bugs. Rather than trial-and-error, effective debugging relies on systematically inspecting code, checking assumptions, and narrowing down where the program’s behavior diverges from expectations.
...
Hint: you can throw an error with the `stop()` function and raise a warning with the `warning()` function.
:::
...
|__Unvalidated__| Bug-free implementation of a flawed idea | Don't go here 💀|
