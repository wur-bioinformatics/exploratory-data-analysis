...
A simple but effective way to start exploring a dataset is to compute a few summary statistics. In the previous week we already encountered a few summary statistics: we looked at e.g. the numbers of rows and columns of a table. In @sec-summary-statistics we will explore a few numerical summary techniques, such as computing the mean and standard deviation of a numeric variable, or counts/frequencies of a categorical variable. While doing so, we will encounter one of the more powerful features of using a programming language: the possibility to create functions. In section @sec-functions we explore this concept a bit further, we highlight a few other commonly occurring programming patterns, and we discuss the relevance of these concepts in the context of reproducible research.
...
## Functions and other programming concepts {#sec-functions}

In the previous chapters, and in @nte-mean-implementation-example, @exr-variance, and @exr-quantiles in particular, we have encountered _functions_. So far, we have used these functions to our advantage, but we have glossed over what a function is exactly, and when it is most appropriate to use functions. In this section we go into a bit more depth to highlight the most important practical aspects of functions in R, whilst avoiding unnecessary theoretical detail as much as possible. We'll discuss a few other relevant programming concepts as well, again mostly limiting ourselves to practical aspects.

### What is a function?
A function can be concisely described as a reusable piece of code. Some clear examples of this are @exr-variance and @exr-quantiles: you implemented some logic to perform a calculation, and then later executed that logic in one line of code when you executed the function.
...
TODO: discuss - functions - scope - OOP (classes, methods, etc.)
