...
First load the proper libraries and filter/transform the data using the functions that you created yesterday
...
We can use the 2-gene representation of the samples to find clusters of similar samples (like you already did on day 2 of the course). Hierarchical clustering starts with each sample in its own cluster and then step-wise merges the two most similar clusters based on a distance measure (Euclidean, Correlation-based, etc.) until all samples are in one cluster. This is typically depicted as a This merging can be stopped if a certain number of clusters is reached. How the distance is calculated depends on the distance measure, but also on the point in a cluster to calculate the distance from.For this several methods are used that give different results. With **average linkage**, the distance between two cluster is calculated as the average of all distances between the members of both cluster. In **complete linkage**, the largest distance between any two members of two cluster is used, while **single linkage** uses the smallest distance between any two members of both clusters. You can imagine that both the distance measure as well as the linkage methods affect the clustering.
...
plot(hc_samples, hang=-1) # this actually calls plot.hclust, hang=-1 makes sure the tree starts (ends?) at 0
```

This should show a tree (upside down?!) with two very distinct branches of samples. The height value on the y-axis is actually the distance between clusters. We can check this by looking at the content `d_samples`. The distance between samples 11 and 12 is a bit more that 12 and that is also where the horizonal line is that merges both clusters. The root of the tree is slightly higher than 70, which agrees with the largest distance between any pair of samples (try `max(d_samples)`) which is 73.6.

To get the actual clusters, we can cut the tree at a certain height. Alternatively you can specify the number of clusters you want.
...
Now we can add the cluster numbers to the data frame and annotate the figure with these to check that the clustering worked. The stress type is shown as the shape of the sample.
