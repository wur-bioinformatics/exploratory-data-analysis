...
In the following days we'll cover a couple of file-types that are common, especially tomorrow, ([Chapter 2](/week1/day2.qmd) we will cover file-types.
...
::: {.callout-note appearance="simple"}
#### Boxplot
![A boxplot of the Col-0 data from the Warmerdam paper](Week1_day1_Boxplot.png)
...
::: {#exr-histogram}
#### Histogram
Try to plot a histogram, for that you need to adjust the boxplot code above a bit.
...
#| code-fold: false
        ...
        ggplot2::ggplot(pheno_data) + aes() + ###select the right set of values to make the plot (hint: you only need the quantitative data)
...
::: {#exr-table}
#### Make a table 
Now use `table()` to check the Line names. For this you need to change the column that is accessed

```{r w1d1_table_for_lines_ASSIGNMENT, include = TRUE,eval=FALSE}

    table(pheno_data$) ### you need to change the column you access
...
You now conducted two types of analysis that tell you whether the data is normally distributed. (1) what do you see in the qqplot, and what does it mean? (2) what is the p-value from the Shapiro-Wilk test? (3) what does a p-value mean?
:::
...
Now the data is filtered, we can test.
...
The function I use most often is `pdf()`.
