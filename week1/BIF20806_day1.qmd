...
## Exploratory Data analysis in R - week 1, day 1: Why data analysis is so important
...
In order to illustrate some concepts, we will make use of data from this paper:
...
Now, check the message you see in the console. The R version displayed is important. The script you are using should work with R versions 4.3 and onwards. If your version number is lower, please first shut-down RStudio and install a newer version of R. You can find out how to do so [here](https://support.wur.nl/esc?id=kb_article&sysparm_article=KB0010713&table=kb_knowledge&searchTerm=r%20installation).
...
#### Telling R where on the harddrive it should work from.
First, you need to set a work directory (tell R where on the harddrive you'll be working from). This can be done with the function `setwd()`, where you have to fill in the work directory yourself. If you want to know how to use that function type `?setwd()` into the console.
...
In this tutorial we will be making use of various packages. Before we start, make sure you have the following packages installed:
...
Note that you only need to install a package once, and only re-install it if the package has a new version or you install a new version of R. To activate the package, we need to load it into memory:
...
During this course, we will load different types of data files into R. This is an essential skill for working with real-world data. In the following days we'll cover a couple of file-types that are common. For today, we have provided you with data in tab-delimited text format. Try to load this data using the funcion `read.delim()`.
...
The code below produces a boxplot, great for checking continuous data. I notice in the plot that there are a couple of single points (the horizontal stripes) per strain, also, there seem to be a couple of really high values. Let's check these.
...
Let's have a look at the distribution of the data. Try to plot a histogram, for that you need to adjust the code above a bit. You need to use the `geom_histogram()` function. Use the [ggplot2 website](https://ggplot2.tidyverse.org/) to figure out how to make this plot.
...
For the single values per genotype, it can be handy to use the `table()` function. Try to use this function on the column _genotype_. You can use `pheno_data$genotype` to access that column.
...
##### Normal distribution 
Let's figure out whether the data is normally distributed. For this we can use the functions `stat_qq()` and `stat_qq_line()`.
...
##### T-test
Now we can use a t-test to compare between two samples. For that, we first need to generate a smaller set of genotypes. We want to test a genotype versus the _A. thaliana_ reference genotype Col-0. Let's take the also popular Cvi-0 genotype to compare.
...
Now we can test. See if you can test whether these two _A. thaliana_ genotypes are different in their susceptiblity to _Meloidogyne incognita_. Use `t.test()`
...
We can make it a bit fancier by plotting the p-value as well. Try to include it into the plot by using the function `annotate()`.
...
##### Saving plots
Now you have your plot, you need to save it. This also requires attention. There are multiple 'graphical devices' in `R` that you can use to plot. 

The function I use most often in `pdf()`. The reason is that this function produces vectorized pdf's, which means that figures have infinite resolution. When you want to save your plot using this function, either save the figure in a file (as in the previous section), or plot directly within the 'graphical device', if you use {ggplot2}, you often have to explicitly define that you print the plot, by adding the function `print()`. Your entire plotting code than should be included in the brackets.
...
```{r w1d1_for_loop_check2, include = TRUE, eval=FALSE, class.source = 'fold-show'}
    ...
    for(i in c("N. americanus","E. vermicularis","M. incognita","A. suum","T. semipenetrans","O. volvulvus","D. dipsaci","E. arcuatus")){
        print(i)
    }
...
Now, let's automatize the t-test function. This is a bit simpler than plotting. For this, you need to make a for-loop cycling over the genoypes in `pheno_data`. You can remove Col-0 from the vector using `genotypes <- genotypes[genotypes != "Col-0"]`. Now, with what you read above, write a for-loop doing the t-test of each genotype versus Col-0 and make sure to pint the values in the console (either use `cat()` or `print()`).
...
Now take the plotting function we wrote before and use it to save plots into an object. Note that this takes a while to run (~2 minutes).
