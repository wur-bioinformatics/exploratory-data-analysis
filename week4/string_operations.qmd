...
This can be a useful approach if you want a specific order - we already encountered this when trying to visualise the geographic distribution of admixture groups in the same order as the publication (@wk4d2_mapview).
...
## The argument starts with % and ends with d (for integers). In between we tell the function to pad with zeros up to 2 digits. If we exceeded 100 genotypes we would have needed "%03d" etc. 
...
::: callout-note
You have already encountered the `apply` family of functions in earlier weeks, for example in week 2 when you learned about @functions.
