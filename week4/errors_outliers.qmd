...
The first of these is simply the four-character (filetag) of the file you analysed. Make sure you type this correctly! Typos here wil mean your data will most likely be filtered out and not used in the plenary analysis.

The number of genotypes and blocks should be fairly straigtforward to work out (have you already used the function `unique()`? If not, check it out!), as well as the mean trait values for trait1 and trait2 (the function `mean()` might help!).
...
Note that in the exam you **will** be expected to be able to write a simple function, including the corrct use of input arguments in round brackets, function body in curly brackets, `return` at the end etc.
